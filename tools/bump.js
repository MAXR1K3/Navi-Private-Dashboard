...
/* tools/bump.js — 一条命令搞定发版前的两件杂事：
     ① 把 sw.js 的 SHELL 预缓存列表按 index.html 的实际样式与脚本重新生成；
     ② 把版本号（sw.js 的 CACHE、SHELL 里的样式、index.html 的样式）一起 +1。
...
html = html.replace(/(css\/[A-Za-z0-9._/-]+\.css)\?v=\d+/g, `$1?v=${next}`);
const styles = [...html.matchAll(/<link rel="stylesheet" href="([^"]+)"/g)].map(m => "./" + m[1]);
const scripts = [...html.matchAll(/<script src="([^"]+)"/g)].map(m => "./" + m[1]);
const head = ["./", "./index.html", "./manifest.webmanifest", ...styles];
...
console.log(`✔ 版本 navi-v${cur} → navi-v${next}（sw.js CACHE / SHELL / index.html 的样式）`);
console.log(`✔ SHELL 已按 index.html 收录：${styles.length} 个样式表`);
